...
import type {
  SymbolDef, SymbolRole, SymbolRow, CallerRow, CalleeRow, StatsRow,
  RouteRow, ExternalDepRow, ConfigKeyRow, FileChurnRow, SymbolHistoryRow, SymbolHistoryInsert,
} from '../types.js';
import {
  Row, toNum, toStr, toNullStr, toNullNum, escapeLike, toSignedI64, toUnsignedI64,
  isRankableKind, buildRoleFilter, resolveSearchFlags, splitIdentifierTokens,
  symbolSelectCols, toSymbolRow, makeSymbolKey, symbolNameVariants, ftsQuery,
  resolveImportToFileId, normalizePath,
  EdgeResolutionStats, FileClassification, SymbolSearchOptions, StoreOptions, SchemaInfo,
} from './store-helpers.js';
import { runMigrations, hasColumn } from './store-migrations.js';

// Re-export the public helper surface so existing importers of these names from
// '../db/store.js' keep resolving after the move into store-helpers.ts.
export {
  isRankableKind, splitIdentifierTokens, makeSymbolKey, symbolNameVariants, ftsQuery,
} from './store-helpers.js';
export type {
  EdgeResolutionStats, FileRole, FileClassification, SymbolSearchOptions, StoreOptions, SchemaInfo,
} from './store-helpers.js';
...
export class Store {
  ...
  constructor(dbPath: string, options: StoreOptions = {}) {
    ...
    this.hasComplexityColumns = hasColumn(this.db, 'symbols', 'cyclomatic');
    this.hasV4Tables = this.checkHasV4Tables();
    this.hasSymbolRoleColumn = hasColumn(this.db, 'symbols', 'symbol_role');
    this.hasModuleTables = this.checkHasModuleTables();
    this.hasV7Columns = hasColumn(this.db, 'symbols', 'provenance') && hasColumn(this.db, 'symbols', 'shape_hash');
  ...
  listRoutes(options: {
    ...
    if (!this.hasV4Tables) return [];
    const hasProtocol = hasColumn(this.db, 'routes', 'protocol');
    const where: string[] = [];
  ...
  getSymbolHistoryWatermarks(
    ...
    if (!hasColumn(this.db, 'symbol_history_progress', 'file_hash')) return out;
    const rows = this.db.prepare(`
  ...
  clearSymbolHistoryWatermarks(repoRoot: string): void {
    if (!hasColumn(this.db, 'symbol_history_progress', 'file_hash')) return;
