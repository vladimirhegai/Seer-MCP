...
function agentsBlock(): string {
  return [
    ...
    '## Seer',
    '',
    'Seer maps this repo (symbols, callers, tests, routes, boundaries, risk, and',
    'per-symbol git history) so you understand the impact of a change before you',
    'make it. Reach for the `seer_*` MCP tools BEFORE `rg`, file reads, or broad',
    'search whenever a task touches code structure — they cost fewer tokens and',
    'round-trips than searching by hand. Every tool self-describes its full',
    'arguments over MCP, so this guide is about WHEN to use which, not a full',
    'reference.',
    '',
    'Required workflow before editing code:',
    '1. Call `seer_health` once; confirm `workspace` is this repo.',
    '2. If `seer_health.workspace` is not this repo, stop using Seer and tell the',
    '   user to restart/reload the agent after running `npx seer-mcp init --yes',
    '   --force` here. Do not use stale Seer results for the task.',
    '3. If `seer_health` is unavailable or times out, say the MCP session is',
    '   stale/unavailable; do not inspect Seer config files or trust stale results.',
    '4. Know the target symbol? Call `seer_preflight { symbol, file }` (or',
    '   `seer_context { symbol, file }`) before reading files. Do not know it?',
    '   Call `seer_search`, then `seer_definition` / `seer_file_symbols` on the hit.',
    '5. For common names, pass `file` (or a qualified `Class.method`). On a bare',
    '   ambiguous name the response carries a compact `nameAmbiguity` hint with',
    '   the top match and alternative count — re-call with `file` if it picked wrong.',
    '',
    'Pick the tool by what you need:',
    '',
    '| You need | Call |',
    '|---|---|',
    '| Confirm Seer is live for this repo | `seer_health` |',
    '| Find a symbol/file you cannot name | `seer_search` → `seer_definition` / `seer_file_symbols` |',
    '| Everything before editing a known symbol | `seer_preflight { symbol, file }` (or `seer_context`) |',
    '| Impact of changes already in the working tree | `seer_preflight { range: true }` |',
    '| Direct callers / callees | `seer_callers` / `seer_callees` |',
    '| Full transitive blast radius | `seer_trace { scope: "callers" }` (`mode: "summary"` for big graphs) |',
    '| Real argument patterns before writing a NEW call | `seer_callers { includeSnippets: true, limit: <small> }` |',
    '| Tests that cover a symbol | `seer_behavior` |',
    '| How a symbol changed over time | `seer_history` |',
    '| What else tends to change with it | `seer_changes_with` (needs full history index) |',
    '| Shape of a big file before reading it | `seer_skeleton { file }` |',
    '| Orient in an unfamiliar repo | `seer_architecture` / `seer_modules` / `seer_boundaries` |',
    '| Many read-only lookups at once | `seer_batch` (up to 25) |',
    '| Literal strings, comments, config values, docs | `rg` (NOT Seer) |',
    '',
    'Worked example — task: "make `chargeCard` also accept an idempotency key":',
    '1. `seer_preflight { symbol: "chargeCard", file: "billing/charge.ts" }` — one',
    '   packet: definition, direct + transitive callers, covering tests, recent',
    '   history, route/boundary exposure, and a risk verdict. Read this before files.',
    '2. You are changing the signature, so `seer_callers { symbol: "chargeCard",',
    '   file: "billing/charge.ts", includeSnippets: true, limit: 8 }` — see how each',
    '   call site actually passes arguments, and update them to match.',
    '3. `seer_behavior { symbol: "chargeCard" }` — the tests to update and run.',
    '4. `seer_changes_with { symbol: "chargeCard" }` — symbols that historically',
    '   change in the same commits (e.g. a `ChargeRequest` type or a webhook',
    '   handler) that no call edge links. Advisory: treat as "also check these".',
    '5. `rg` for string/route/config references the call graph cannot see, then edit.',
    '6. After editing, `seer_preflight { range: true }` to confirm the working-tree',
    '   blast radius before you summarize.',
    '',
    'Notes:',
    '- `seer_callers` `total` = call SITES; `uniqueCallers` = distinct functions.',
    '- Some Seer results include `precision`, `warnings`, and `nextBestCall`; use',
    '  `nextBestCall` as the deterministic follow-up when it is present.',
    '  For C/C++ member calls (`obj->method()`) the receiver type is unresolved, so',
    '  a low resolved count is reported with `ambiguity.likelyCallersEstimate` —',
    '  narrow it with `groupByFile`, `filterReceiverType`, or `includeNameMatches`',
    '  (or import a SCIP index for an exact count).',
    '- `seer_history` auto-builds just the queried symbol\'s file (~1s) on a cold',
    '  miss. The FULL index (`seer_symbol_history_build` with no args, or',
    '  `seer symbol-history`) can take minutes on large repos — ask the user first.',
    '  `seer_changes_with` reports `historyComplete: false` until that full index',
    '  exists; any partners then may be partial or falsely empty.',
    '- Treat `seer_behavior` `lowConfidence` / `testCoverageState: "heuristic-only"`',
    '  as a hint, not proof of coverage.',
    '',
    'Use `rg` or file reads after Seer for literal strings, comments, docs, config',
    'values, unsupported languages, or when Seer returns no useful hit from the',
    'correct workspace.',
    '',
    'Do not read `.agents/mcp_config.json`, `.mcp.json`, `.cursor/mcp.json`, or',
    'other MCP config files during normal code tasks — only when the user asks to',
    'install, update, uninstall, or debug Seer. Do not run `npx seer-mcp` as a',
    'substitute for the MCP tools when they are available.',
    AGENTS_END,
  ].join('\n');
}
