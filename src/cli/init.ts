...
function agentsBlock(): string {
  return [
    ...
    '6. For common method names, pass `file` to the symbol tools (`seer_context`,',
    '   `seer_callers`, `seer_callees`, `seer_behavior`, `seer_risk`, `seer_history`,',
    '   `seer_trace` callers) so Seer uses the exact symbol definition. When a bare',
    '   ambiguous name resolves to one definition (`seer_context`, `seer_behavior`,',
    '   `seer_risk`, `seer_trace` callers/callees), the response carries a',
    '   `nameAmbiguity` hint naming the picked definition and the alternatives —',
    '   re-call with `file` if it picked the wrong one.',
    ...
    '',
    'Blast radius / refactor impact (e.g. "what breaks if I change this symbol?"):',
    '1. `seer_context { symbol, file }` — definition, callers, callees, blastRadius,',
    '   tests, and risk in one packet. Start here.',
    '2. `seer_callers` = DIRECT callers; `seer_trace` callers = TRANSITIVE blast',
    '   radius. `total` counts call SITES; `uniqueCallers` counts distinct caller',
    '   functions. Pass `file` (or a qualified `Class.method`) to pin the exact symbol.',
    ...
    '   call graph cannot see (e.g. script-exposed engine APIs).',
    'Argument caps: `seer_context` caller/callee/test/affected limits ≤ 100,',
    'history ≤ 50, `callerDepth` ≤ 6; `seer_trace` callers `maxDepth` ≤ 8.',
    '',
