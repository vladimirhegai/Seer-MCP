...
export function mcpInstructions(): string {
  return [
    ...
    'To assess changes already in the working tree, call seer_preflight with range=true (or fromRef/toRef) for the touched-symbol blast radius.',
    ...
    'Before writing a NEW call to an unfamiliar function, call seer_callers with includeSnippets=true (and a small limit) to see real argument/usage patterns at existing call sites.',
    'After locating an edit target, call seer_changes_with to find symbols that historically change together with it (advisory coupling the call graph cannot see). Check historyComplete: when false, partners may be partial or falsely empty; when true, still treat partners as hints to check, not proof.',
    ...
    'For C/C++ member calls where seer_callers reports an ambiguity, treat likelyCallersEstimate as bounded call-site evidence, then narrow with seer_callers groupByFile=true (where the sites concentrate) and filterReceiverType (best-effort receiver class; true infers it); includeNameMatches pages the raw list. SCIP import gives an exact count.',
    'When a result includes nextBestCall, prefer that follow-up before guessing the next Seer query.',
